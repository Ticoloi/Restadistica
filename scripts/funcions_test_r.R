...
summarise(consum_aigua, n = n(), n.na = sum(is.na(`Codi comarca`)))

# Compta nombre de files
summarise(consum_aigua, n.cat = n_distinct(`Codi comarca`))
...
ggplot(consum_aigua %>% filter(`Comarca` == "BAIX CAMP, EL"),
       aes(y = Any, x = `Domèstic xarxa`)) +
  geom_point(alpha = 0.5, color = "darkblue") +
  coord_flip() +
  labs(title = "Consum per any a la comarca BAIX CAMP", y = "Any", x = "Domèstic xarxa") +
  theme_minimal()
...
ggplot(data_baix_camp, aes(x = Any, y = `Domèstic xarxa`)) +
  ...
  labs(title = "Consum per any a la comarca BAIX CAMP", x = "Any", y = "Domèstic xarxa (Total)") +
  theme_minimal()
...
taula_girona %>%
  pivot_longer(
    cols = c(Domèstic_total, Activitats_total),
    names_to = "Categoria",
    values_to = "Consum"
  ) %>%
  mutate(
  ...
  )  # Giro els anys per llegibilitat
...
taula_girona %>%
  ...
  scale_y_continuous(labels = scales::comma) +
  scale_color_manual(values = c("PIB" = "darkblue", "Poblacio" = "darkorange")) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", size = 14),
    legend.position = "bottom",
